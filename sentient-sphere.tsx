...
function Sphere() {
  ...
  const fragmentShader = `
    ...
    void main() {
      // warm near-black ink color (#1E1613)
      vec3 color = vec3(0.118, 0.086, 0.075);

      float intensity = 0.45 + vDisplacement * 2.5;

      float line = smoothstep(0.0, 0.02, abs(fract(vUv.x * 20.0) - 0.5));
      line *= smoothstep(0.0, 0.02, abs(fract(vUv.y * 20.0) - 0.5));

      // dark lines fade softly, denser where displaced
      float alpha = clamp(intensity * (1.0 - line * 0.6), 0.0, 1.0) * 0.32;
  ...
  if (!mounted) {
    return (
      <div className="w-full h-full flex items-center justify-center">
        <div className="w-64 h-64 rounded-full border border-foreground/10 animate-pulse" />
      </div>
    )
  }
