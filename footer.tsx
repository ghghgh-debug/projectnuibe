...
export function Footer() {
  ...
  return (
    <footer id="contact" className="relative">
      ...
      {/* Footer Info */}
      <div className="px-8 md:px-12 py-8 border-t border-foreground/10">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
          {/* Local Time */}
          <div className="font-mono text-xs tracking-widest text-muted-foreground uppercase">
            <span className="mr-2">Seoul</span>
            <span className="text-foreground tabular-nums">{time}</span>
          </div>

          {/* Links */}
          <div className="flex gap-8">
            {["Instagram", "TikTok", "Telegram"].map((link) => (
              <a
                key={link}
                href="#"
                data-cursor-hover
                className="font-mono text-xs tracking-[0.12em] uppercase text-muted-foreground hover:text-foreground transition-colors duration-300"
              >
                {link}
              </a>
            ))}
          ...
          {/* Copyright */}
          <p className="font-mono text-xs tracking-widest text-muted-foreground lowercase">© {new Date().getFullYear()} nuibe</p>
        </div>
      </div>
    </footer>
  )
